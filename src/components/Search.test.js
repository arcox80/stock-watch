...
describe('<Search />', () => {
  ...
  it('It should fire the dispatches when the form is submitted', () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(fetchStockRequest());
    expect(dispatch).toHaveBeenCalledWith(fetchHeadlinesRequest());
    //expect(dispatch).toHaveBeenCalledWith(mockFetchStockAction);
    //expect(dispatch).toHaveBeenCalledWith(mockFetchHeadlinesAction);
  });
